...
This rulebook monitors RACF whenever a user, defined to SYS1.UADS but not to RACF, successfully logs on to TSO or submits a batch job not defined to RACF from another system. The event generated by zSecure is alert C2P1101I and delivered through Kafka.
...
The rule fires immediately when any one incoming event satisfies the condition. 
...
* Verify the C2P1101I events are being published to Kafka.
