...
This rulebook monitors RACF whenever a user gains access of UPDATE or higher to a sensitive data set. The event generated by zSecure is a C2P1213I alert and it is delivered through Kafka.
...
The rule fires immediately when any one of the incoming events satisfies the condition.
There is no correlation window because each zSecure alert is self-contained and no second event is required.

Action
~~~~~~

Launches the AAP workflow template ``EDA - Alert 1213 Response Workflow`` in the Default organization. The matched event is passed to the workflow template through ``ansible_eda.event``. The response performed by the workflow template is documented on the corresponding playbook page in this collection.
...
* Verify the C2P1213I events are published to Kafka.
