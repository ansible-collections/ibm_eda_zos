...
.. code-block:: yaml
   ...
   - name: Rule to handle alert 1213 - Access >= UPDATE on sensitive data set
     hosts: all
     sources:
       - name: kafka
         ansible.eda.kafka:
         ...
         filters:
           - ibm.ibm_eda_zos.security_alerts:
               event_source: "kafka"
     rules:
       - name: "Handle alert 1213 - Access >= UPDATE on sensitive data set"
         condition: event.body.alert_code == "C2P1213I"
         action:
           run_workflow_template:
             name: "EDA - Alert 1213 Response Workflow"
...
**Handle alert 1213 – Access >= UPDATE on sensitive data set**
...
.. code-block:: python
   ...
   event.body.alert_code == "C2P1213I"
...
Launches the AAP workflow template ``EDA - Alert 1213 Response Workflow`` in the Default organization. The matched event is passed to the workflow template through ``ansible_eda.event``. The workflow response is documented on the corresponding playbook page in this collection.
...
.. code-block:: json

   {
     "body": {
       "alert_code": "C2P1213I",
       "alert_message": "C2P1213I UPDATE access by USER001 on site-sensitive UPDATE data set DATA.SET.NAME",
       "hostname": "ZSYS01",
       "action_user": "USER001",
...
* The rulebook runs continuously and monitors the Kafka topic for new events.
...
* Verify that C2P1213I events are published to Kafka.
